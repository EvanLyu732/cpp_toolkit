...
#else
#error \
    "cxx version is not supported std 17!!! Please use higher version cxx compiler"
#endif
...
struct Attributes {
  // spdlog pattern
  static inline constexpr char* pattern =
      "%^|%Y/%D/%H:%M:%S|:|%l|:|%s:%#| %v %$";
...
struct __attribute__((visibility("default"))) networksink
    : public spdlog::sinks::base_sink<Mutex> {
  networksink(int bind_port, int send_port = 6999)
      : send_ep_(::boost::asio::ip::udp::endpoint(::boost::asio::ip::udp::v4(),
                                                  send_port)),
        service_(::boost::asio::io_service()),
        socket_(::boost::asio::ip::udp::socket(
            service_, ::boost::asio::ip::udp::endpoint(
                          ::boost::asio::ip::udp::v4(), bind_port))) {
    std::lock_guard<std::mutex> lock(Mutex);
    this->formatter_ =
        std::make_unique<spdlog::pattern_formatter>(Attributes::pattern);
    initialColorLevel();
    service_.run();
  }

  ~networksink() { socket_.close(); }
 ...
 protected:
  void sink_it_(const ::spdlog::details::log_msg& msg) override {
    ...
    socket_.async_send_to(
        ::boost::asio::buffer(sink_color.data(), sink_color.size()), send_ep_,
    ...
    socket_.async_send_to(
        ::boost::asio::buffer(formatted.data(), formatted.size()), send_ep_,
        [this](::boost::system::error_code ec, std::size_t length) {
          if (ec) {
            return;
          }
...
void setup_config() {
  ...
  Config::bind_port =
      config->get_qualified_as<int>("spdlog.BIND_PORT").value_or(6688);
  Config::send_port =
      config->get_qualified_as<int>("spdlog.SEND_PORT").value_or(7799);
  Config::log_module =
      config->get_qualified_as<std::string>("spdlog.LOG_MODULE")
          .value_or("undefined");
  Config::log_files_dirs =
      std::string(current_path.string() + "/logs/" +
                  std::string(Config::log_module) + ".log");
  Config::logger_name =
      std::string(std::string(Config::log_module) + "/_logger");
}

void initial_logger() noexcept {
  if (!is_init) {
    setup_config();
    spdlog::init_thread_pool(Attributes::thread_pool_size,
                             Attributes::thread_pool_nums);
    using networksink_mt = networksink<std::mutex>;
    auto network_sinks =
        std::make_shared<networksink_mt>(Config::bind_port, Config::send_port);
    auto rotating_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
        std::string(Config::log_files_dirs), 1024 * 1024 * 10, 3);
    std::vector<spdlog::sink_ptr> sinks{network_sinks, rotating_sink};
    auto logger = std::make_shared<spdlog::async_logger>(
        std::string(Config::logger_name), sinks.begin(), sinks.end(),
...
#define LOG_INFO(...)                                                  \
  if (!ud::tools::log::is_init) {                                      \
    ...
    auto logger =                                                      \
        spdlog::get(std::string(ud::tools::log::Config::logger_name)); \
